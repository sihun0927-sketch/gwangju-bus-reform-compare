/**
 * 배차간격 추론 층 — 「계산중」 뒤에 오는 값을 만든다 (ADR-0011).
 *
...
/**
 * 시간 상한 둘. **카드가 답하는 데 걸리는 시간은 이 둘이 정한다 — 약속은 5초다.**
 *
 ...
 *
 * **무엇을 내주고 얻은 것인가.** 마감을 낮추면 늦게 오는 표본이 버려지므로 중앙값을 잡을
 * 표본이 준다. 셋이 다 오면 가운데를 잡고, 하나만 와도 밴드에 가둔 그 값을 쓰고, 하나도
 * 안 오면 계산값을 보인다 — **어느 쪽이든 5초 안에 답이 뜬다.** 깨끗하게 잰 값으로 요청
 * 하나가 2.1초였으니 대개는 셋이 다 온다. 흔들림 없는 값이 더 중요하면 `/headway?route=`가
 * 늘 계산값을 준다.
 */
const TIMEOUT_MS = 4_200;
// 마감은 표본 상한보다 조금만 뒤다 — 상한에 걸린 표본이 정리될 틈만 준다.
// **카드는 5초 안에 답한다**가 이 두 값이 지키는 약속이다
const DEADLINE_MS = TIMEOUT_MS + 300;
...
export function why(record, inferred) {
  ...
  const 근 = record.근거;
  // 「수요」·「중앙」·「총량」처럼 넓은 말은 안 쓴다. 처음 보는 사람은 그 말이 무엇을 센 것인지
  // 모른다 — 무엇을 몇 번 센 것인지 그 자리에서 적고, 문장마다 결론으로 끝낸다.
  //
  // 노선 이름을 주어로 세우지 않는다. 「운림54이/가」처럼 조사가 숫자 읽기에 따라 갈리는데
  // 그것을 코드가 맞힐 수 없다. 상자가 그 줄 곁에 있으므로 주어는 이미 분명하다
  const 이어받은곳 =
    record.대체한노선.length > 0
      ? `개편 전 ${record.대체한노선.join(" · ")}의 길이 나르던 하루 운행량을 이어받은 것입니다`
      : "이어받을 개편 전 노선이 비교표에 없어, 이 노선이 지나는 정류장들의 개편 전 하루 운행량으로 잡은 것입니다";
  const 종류 = record.종류 ? `${record.종류} 노선 ${근.종류노선수}개` : `번호만 있는 노선 ${근.종류노선수}개`;
  const 가운데 = `${종류}의 가운데 배차가 ${Math.round(근.종류중앙배차)}분인데`;
  // 확신이 낮은 노선은 점 추정이 없다. 밴드 한가운데를 지어내 견주면 없는 값을 말하는 셈이다
  const 견줌 =
    record.배차간격 === null
      ? `${가운데} 이 노선은 나누는 방식에 따라 ${Math.round(record.밴드[0])}~${Math.round(record.밴드[1])}분으로 갈려 한 값으로 말하지 않습니다`
      : (() => {
          const 배 = 근.종류대비;
          const 내값 = Math.round(record.배차간격);
          if (배 < 0.8) return `${가운데} 이 노선은 ${내값}분이라 그보다 촘촘합니다`;
          if (배 <= 1.25) return `${가운데} 이 노선은 ${내값}분이라 그와 거의 같습니다`;
          if (배 < 2) return `${가운데} 이 노선은 ${내값}분이라 그보다 성깁니다`;
          return `${가운데} 이 노선은 ${내값}분으로 그 ${배}배만큼 성깁니다`;
        })();
  return (
    `하루 ${Math.round(근.운행횟수)}번(상·하행 합계) 달린다고 보고 나눈 값입니다.` +
    ` 그 횟수는 ${이어받은곳}.` +
    ` ${견줌}.` +
    ` 시가 공표한 것은 노선별 배차가 아니라 하루 운행횟수 합계(${record.망.운행횟수[0]}→${record.망.운행횟수[1]}번)뿐이라,` +
    ` 이 수는 그 합계를 노선마다 갈라 낸 추정입니다.`
  );
}
