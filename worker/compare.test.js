...
test("배차간격은 개편 전 카드에만 숫자로 서고 개편 후는 「정보 없음」이다", async () => {
  ...
  const 글 = await 비교(전남대, 광주송정역);
  const 개편_전 = 카드(글, "before");
  assert.match(개편_전, /배차간격 12분 · 2개 정류장/, 개편_전);
  assert.match(개편_전, /배차간격 7분 · 17개 정류장/, "구간마다 그 노선의 값이다");
  assert.ok(!개편_전.includes("배차간격 정보 없음"), "개편 전 두 노선 모두 값이 있다");

  // 환승 경로의 지표 표 — 구간 둘의 값이 차례대로 한 칸에 선다(CONTEXT 「지표」)
  assert.equal(지표(개편_전, "배차간격"), "12분 · 7분", "구간 차례 그대로 잇는다");
  assert.match(지표(개편_전, "노선번호"), /^.+ · 좌석02$/, "노선번호도 같은 차례다");
...
test("전남대 → 광주송정역은 개편 전 환승 없는 길이 있어도 환승 쪽이 카드에 선다", async () => {
  // #24까지 이 쌍의 개편 전 카드는 「환승 없음 좌석02 · 8분 · 16곳 · 도보 175m」였다. 환승이 들어오면서
  // 「환승 1회 첨단30 → 좌석02 · 7분 · 19곳 · 도보 20m」로 바뀐다 — 전남대 앞에서 바로 타 도보가
  // 164m → 9m로 줄기 때문이다. 추정 소요 시간에 **환승 대기가 0초**라 이런 뒤집힘이 생기고,
  // 그것이 스펙이 시킨 순위다(「순위가 추정 소요 시간을 따른다」). 바뀐 값을 여기에 박아 둔다
  ...
  assert.ok(개편_전.includes("첨단30") && 개편_전.includes("좌석02"), "갈아타는 두 노선");
...
test("환승 없는 길이 있어도 환승 경로가 빠르면 그쪽이 기본 경로다", async () => {
  ...
  // 송암31로 갈아타면 6분이다. 순위는 추정 소요 시간을 따르므로 카드에 서는 것은 환승 쪽이다
