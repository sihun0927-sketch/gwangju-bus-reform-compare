...
test("지표 표에 전체 값 셋만 있고 배차간격·정류장 수는 경로 줄에 있다", async () => {
  // 값이 사는 자리가 갈린다(CONTEXT 「지표」·「경로 줄」) — 노선마다 달라지는 값은 그 노선 자리에,
  // 경로 전체에 하나씩인 값만 표에. 나누지 않으면 환승 2회 경로에서 같은 값이 네 번까지 겹친다
  const 글 = await 비교(전남대, 광주송정역);
  const 개편_후 = 카드(글, "after");
  assert.equal(지표(개편_후, "예상 시간"), "7분");
  assert.equal(지표(개편_후, "환승"), "없음");
  assert.equal(지표(개편_후, "도보 합계"), "20m");
  assert.equal(지표(개편_후, "노선"), undefined, "노선은 지표가 아니라 경로 줄에 있다");
  assert.equal(지표(개편_후, "배차간격"), undefined, "배차간격도 마찬가지다");
...
test("배차간격은 개편 전 카드에만 숫자로 서고 개편 후는 「정보 없음」이다", async () => {
  // 시가 공표한 값이 개편 전에만 있다(CONTEXT 「경로 줄」). 값은 노선마다 다르므로 지표 표가 아니라
  // 경로 줄의 그 노선 자리에 붙는다. 「분」은 조각이 붙이고 번들에는 숫자만 있다
  const 글 = await 비교(전남대, 광주송정역);
  const 개편_전 = 카드(글, "before");
