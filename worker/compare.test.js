...
test("전남대 → 광주송정역은 개편 전 직행이 있어도 환승 쪽이 카드에 선다", async () => {
  // #24까지 이 쌍의 개편 전 카드는 「직행 좌석02 · 8분 · 16곳 · 도보 175m」였다. 환승이 들어오면서
  ...
  assert.match(개편_전, /예상 시간[^<]*7분/);
  assert.match(개편_전, /정류장[^<]*19곳/);
  assert.match(개편_전, /도보 합계[^<]*20m/);
  assert.ok(개편_전.includes("첨단30") && 개편_전.includes("좌석02"), "갈아타는 두 노선");
  assert.ok(개편_전.includes("<b>광주역</b>에서 환승 · 같은 정류장"), "환승 자리");
});
