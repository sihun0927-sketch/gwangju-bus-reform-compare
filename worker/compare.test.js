...
test("전남대 → 광주송정역은 개편 전 환승 없는 길이 카드에 선다", async () => {
  // 환승이 들어온 뒤로 이 쌍의 개편 전 카드는 「환승 1회 첨단30 → 좌석02 · 7분」이었다. 순위가
  // 환승 0회를 시간 앞에 두면서(2026-09-04) 다시 「환승 없음 좌석02 · 8분」이다 — 1분 느려도
  // 갈아타지 않는 길이 먼저다. 뒤집힘의 값을 여기에 박아 둔다
  const 개편_전 = 카드(await 비교(전남대, 광주송정역), "before");
  assert.equal(상태(개편_전), "환승 없음", 개편_전);
  assert.equal(지표(개편_전, "예상 시간"), "8분");
  assert.equal(지표(개편_전, "도보 합계"), "175m");
  assert.equal(지표(개편_전, "노선번호"), "좌석02");
  assert.ok(!개편_전.includes("첨단30"), "더 빠른 환승 경로는 기본 카드가 아니다");
});
...
test("환승 경로가 더 빨라도 환승 없는 길이 기본 경로다", async () => {
  // 이십곡리 → 기아자동차는 만연빌딩에서 147m 걸어 송암31로 갈아타면 6분이고, 갈아타지 않으면
  // 228로 17분이다. 11분이 느린데도 카드에 서는 것은 환승 없는 쪽이다 — 순위 ①이 환승 0회다
  // (CONTEXT 「추정 소요 시간」, 2026-09-04). 빠른 환승 경로는 「다른 경로 더 보기」에 남는다
  const 글 = await 비교(이십곡리, 기아자동차);
  const 개편_전 = 카드(글, "before");
  assert.equal(상태(개편_전), "환승 없음", 개편_전);
  assert.equal(지표(개편_전, "예상 시간"), "17분");
  assert.equal(지표(개편_전, "노선번호"), "228(구151.화순전대병원)");
  assert.ok(!개편_전.includes("송암31"), "더 빠른 환승 경로는 기본 카드가 아니다");
  assert.ok(글.includes("다른 경로 더 보기"), "버려지지는 않는다");
});
