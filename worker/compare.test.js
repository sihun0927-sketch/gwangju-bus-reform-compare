...
test("지표 표는 다섯 줄이고 정류장 수만 경로 줄에 남는다", async () => {
  ...
  assert.match(지표(개편_후, "배차간격"), /^약 \d+분$/, "개편 후는 추정값이고 그렇다고 적힌다");
  assert.equal(지표(개편_후, "정류장 수"), undefined, "정류장 수는 경로 줄에만 있다");
  assert.match(개편_후, /배차간격 약 \d+분<details class="why">/, "경로 줄의 노선별 값");
...
test("배차간격은 개편 전이 공표값, 개편 후가 추정값이고 화면이 그것을 가른다", async () => {
  ...
  assert.ok(!/배차간격 약 /.test(개편_전), "공표값에는 「약」을 안 붙인다");

  const 개편_후 = 카드(글, "after");
  assert.match(개편_후, /배차간격 약 \d+분/, "개편 후는 추정값이라고 적는다");
