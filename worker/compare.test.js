...
test("지표 표는 다섯 줄이고 정류장 수만 경로 줄에 남는다", async () => {
  ...
  assert.equal(지표(개편_후, "배차간격"), "정보 없음", "개편 후는 공표된 값이 없다");
  assert.equal(지표(개편_후, "정류장 수"), undefined, "정류장 수는 경로 줄에만 있다");
  assert.match(개편_후, /배차간격 정보 없음 · \d+개 정류장/, "경로 줄의 노선별 값");
...
test("배차간격은 개편 전 카드에만 숫자로 서고 개편 후는 「정보 없음」이다", async () => {
  // 시가 공표한 값이 개편 전에만 있다(CONTEXT 「경로 줄」). 「분」은 조각이 붙이고 번들에는 숫자만 있다.
  ...
  assert.match(카드(글, "after"), /배차간격 정보 없음/, "개편 후는 공표된 값이 없다");
  ...
  // 순환01은 개편 전인데도 배차 CSV에 없는 유일한 노선이다 — 없는 것과 깨진 것을 가른다
