/**
 * 배차간격 답 하나 — 시민이 노선을 물었을 때 LLM이 읽을 순수 함수 (ADR-0010).
 *
...
function forRoute(name, asked) {
  ...
  return {
    ...
    // 같은 차량을 대기시간 총합이 가장 작아지도록 다시 나눈 것(ADR-0010 제곱근 법칙).
 ...
 *
 * 읽는 쪽이 화면이 아니라 LLM이기 때문이다(ADR-0010 결정 4). `?route=`가 없으면 망 전체를 준다.
 */
