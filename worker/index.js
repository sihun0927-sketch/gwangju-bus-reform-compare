 ...
 *
 * `wrangler.jsonc`의 `run_worker_first`가 `/places` · `/compare` · `/journey/…` · `/headway`만 이리로 보내고,
...
import { respond as headway } from "./headway.js";
import { respond as headwayFragment } from "./infer.js";
import { journey } from "./journey.js";

/** `/journey/` 뒤에 오는 경로 키가 시작하는 자리. */
const JOURNEY_PREFIX = "/journey/";

/** `/headway/` 뒤에 오는 개편 후 노선 이름이 시작하는 자리. 카드가 htmx로 부른다 */
const HEADWAY_PREFIX = "/headway/";
...
export default {
  async fetch(request, env) {
    ...
    // 하나뿐인 자료 경로. 조각이 아니라 JSON이라 `htmlResponse`를 안 거친다
    if (pathname === "/headway") return headway(searchParams);
