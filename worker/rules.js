...
/**
 * 배차간격을 모르는 노선의 배차간격(분)을 이 값으로 본다.
 *
 * **오늘 번들에는 그런 노선이 없다** — 마지막 하나였던 순환01도 배차 파일의 A·B를 찾아 붙였다
 * (2026-09-04). 그래도 규칙은 남긴다. 다음 자료에서 한 노선이라도 값을 잃으면 `waitSeconds`가
 * `undefined`로 셈을 하거나 0으로 떨어지는데, 0은 「안 기다리고 탄다」는 뜻이라 그 노선만 순위에서
 * 공짜로 이긴다. 모르는 것을 「좋은 것」으로 읽지 않게 가운데 값을 준다 — 개편 전 공표값의 중앙값이다.
 */
