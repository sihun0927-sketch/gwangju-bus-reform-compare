...
## source/ — 빌드 스크립트가 읽는 입력 CSV 8개

여섯은 시가 공표한 원본(2026-09-02 반입, UTF-8 BOM)이고, `stops.csv` 하나는 광주 BIS API에서
받아 만든 것이며(2026-09-03, ADR-0007), `route_headways.csv` 하나는 **공공데이터 API**에서 받은
개편 전 배차간격 자료다(2026-09-04 반입, ADR-0009). **손으로 고치지 않는다.** 빌드 스크립트는 읽기만 한다 —
...
| `route_headways.csv` | 120 | route_name · headway_minutes | 개편 전 배차간격, 유효 운행시간 보정, 개편 후 배차간격 추정(ADR-0009). **시 공표 아님** — 공공데이터 API |
...
- **배차간격 파일은 다시 받을 절차가 아직 없다.** `stops.csv`에는 `tools/fetch_stops.py` →
  `back_up/api_stops.json` → `tools/build_stops.py`가 있어 언제든 다시 만들 수 있지만,
  `route_headways.csv`는 공공데이터 API에서 받은 것을 손으로 들여왔고 **어느 서비스의 어느 응답인지
  적혀 있지 않다.** 응답 원본도 `back_up/`에 없다. 다시 받아야 할 때 그 자리를 먼저 채운다
  (`tools/fetch_stops.py`가 쓰는 `apis.data.go.kr/6290000/…`과 같은 기관 코드일 가능성이 크지만
  **확인된 것이 아니다**).
- **배차간격 파일의 표기가 또 다르다.** `route_headways.csv`는 순환01을 `순환01A(계수초->시청->조선대)` ·
  `순환01B(…)` 둘로 갈라 적었다 — 노선안은 한 행이다. 노선안에 없는 **마을버스 7행**(`마을700`
  `마을715` `마을720` `마을760` `마을777` 둘 · `마을788`)과 `1187-1`도 섞여 있다. 개편 전 노선안
  111행은 이름이 없으면 그 이름으로 시작하는 줄들의 평균을 쓴다(`headway.headway_of`) — 지금 그
  경우는 순환01 하나뿐이다.
- **배차간격 파일에서 안 쓰는 줄이 있다.** 8394·9355는 시내버스 총량이라 마을버스 줄은 추정에 안
  들어간다. 노선안에 있는 이름만 읽는다.
- **`routes_with_stop_names.csv` · `route_headways_with_stops.csv`는 안 들인다.** 앞의 것은 노선 열과
  `BUSSTOP_NAME` 열이 **같은 행에서 서로 다른 것을 말한다**(노선 111개가 4,746줄에 되풀이되고 이름 열은
  정류장 전체 목록이다 — 표 계산기에서 열을 나란히 붙인 자국이다). 뒤의 것은 배차간격 + 개편 전 노선안
  정류장이라 이 폴더에 이미 있는 둘을 합친 것이다.
