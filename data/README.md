...
**배차간격 원천이 둘이라는 것은 아직 안 정리된 자리다.** 아래 표의 두 줄이 같은 것을 말한다 —
자세한 것과 합칠 방법은 「배차간격 원천이 둘이다」 절에 적었다.
...
- **순환01 한 행이 이 파일에 없다.** 사이트 전체에서 「정보 없음」인 유일한 노선이다 —
  개편 후는 추정값이 다 있다(ADR-0010).
...
## 배차간격 원천이 둘이다 (2026-09-04, 아직 안 정리됨)

같은 자료가 두 파일로 들어와 있다. 합치는 것이 맞지만 화면 문구가 하나 바뀌는 일이라 나중으로 미뤘다.
...
못 쓴다. 반대로 `load.read_headways`를 뒤엣것으로 옮기면 **순환01의 「정보 없음」이 메워진다** —
화면이 좋아지는 쪽이다. 다만 그것은 시민이 보는 문구를 바꾸는 일이라 별도로 검토한다.
