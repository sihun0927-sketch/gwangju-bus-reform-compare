...
def build(source: Path, out: Path, *, align_table: Path | None = None) -> Result:
    ...
    renames = rename_dict.of(load.read_renames(source))
    facts = notes.sources(renames, load.read_removals(source), load.read_additions(source))
    ...
    for pair in pairs:
        ...
        _write_table(out, pair, alignment, before_siblings, after_siblings, renames, facts)
...
def _write_table(
    ...
    renames: rename_dict.RenameDict,
    facts: notes.Sources,
) -> None:
    ...
    up = stop_match.match(pair.before.up, after_up, renames.canon)
    # 하행 칸을 채울지는 뒤집은 뒤의 목록으로 본다. 한쪽이 비면 두 칸을 비우고 비고가 까닭을 말한다
    has_down = bool(pair.before.down) and bool(after_down)
    down = stop_match.match(pair.before.down, after_down, renames.canon) if has_down else []
    ...
    html = render.route_change_table(
        ...
        flipped=flipped,
        notes=notes.for_rows(up, down, facts, table_note=table_note),
    )
