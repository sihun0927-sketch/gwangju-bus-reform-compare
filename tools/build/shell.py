...
탭 전환은 숨긴 라디오 단추와 CSS만으로 한다 — 이 탭은 Worker 없이 돈다(ADR-0001).
아직 못 하는 일을 하는 척하지 않는다: 장소 탭의 입력칸 둘은 `disabled`이고 안내문이 그 까닭을 적는다.
...
입력값으로 바꿔 카드 조각을 부른다(architecture §7-3 Q1·Q2). 표 필터는 없다 — 목록 표는 늘 103행이다.
"""
...
# CDN 스크립트 태그 둘(htmx + 공식 확장 하나). 우리가 쓰는 속성은
# hx-get · hx-target · hx-swap · hx-trigger · hx-ext 다섯뿐이고 우리가 쓴 스크립트는 없다(ADR-0001)
...
def _field(field_id: str, label: str, placeholder: str, attrs: str = " disabled") -> list[str]:
    return [
        '<div class="field">',
