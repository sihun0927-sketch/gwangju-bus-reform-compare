 ...
 * 세면 실측과 배포본이 조용히 갈린다.
 *
 * 정류장 쌍은 `tools/measure_pairs.mjs`가 씨앗을 고정한 난수로 고른다. 다시 돌리면 같은 쌍이
 * 나오므로 값이 재현되고, §6-4(요청당 CPU)가 같은 쌍을 써서 두 §를 나란히 읽을 수 있다.
 */
...
async function main() {
  const 쌍 = 정류장_쌍(Number(process.argv[2] ?? DEFAULT_PAIRS));
