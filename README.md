...
3. `docs/adr/` — 되돌리기 어려운 결정 열하나와 그 이유 (0004는 0007로, 0001의 D1 부분은 0008로 대체됨)
4. `data/README.md` — 원천 10개의 열과 행 수, 정류장 좌표(`source/stops.csv`), 배차간격 원천 둘, 알아 둘 함정
...
개편 후 노선이 물려받게 해 나눈다(ADR-0010). `GET /headway?route=간선18`이 배차간격 추정 · 밴드 ·
...
고정값이 아니라 **부를 때마다 모형이 다시 판단한 추론값**이다(ADR-0011). 매번 다르되
